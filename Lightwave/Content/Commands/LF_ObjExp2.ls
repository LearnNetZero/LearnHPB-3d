...
writemtl:pfilename,pchoice,plum
{
    ...
    for(i=1;i<=mysurfnames.size();i++)
    {
        ...
        //colour
        mymap=getfirstimage(mysurf,SURFCOLR);
        if(mymap)
        {
            myimage=mymap.filename(0);
            mystring="map_Kd "+myimage;
            pfile.writeln(STRING,mystring);
        }
        
        //ambient/Luminosity
        mymap=getfirstimage(mysurf,SURFLUMI);
        if(plum && mymap)
        {
            myimage=mymap.filename(0);
            mystring="map_Ka "+myimage;
            pfile.writeln(STRING,mystring);
        }
        ...
        {
            myimage=mymap.filename(0);
            mystring="map_Ks "+myimage;
            pfile.writeln(STRING,mystring);
        }
        
        //Gloss
        mymap=getfirstimage(mysurf,SURFGLOS);
        if(mymap)
        {
            myimage=mymap.filename(0);
            mystring="map_Ns -imfchan l "+myimage;
            pfile.writeln(STRING,mystring);
        }
        
        //Trans
        mymap=getfirstimage(mysurf,SURFTRAN);
        if(mymap)
        {
            myimage=mymap.filename(0);
            mystring="map_d -imfchan l "+myimage;
            pfile.writeln(STRING,mystring);
        }
        
        //Bump
        mymap=getfirstimage(mysurf,SURFBUMP);
        if(mymap)
        {
            myimage=mymap.filename(0);
            mystring="map_bump -bmp l.0 "+myimage;
            pfile.writeln(STRING,mystring);
        }
